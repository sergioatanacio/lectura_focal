...
export class StartReading {
  ...
  async execute(params: {
  ...
  }): Promise<ReadingViewDTO> {
    const document = await this.documentRepo.findById(params.documentId);
    if (!document) {
      throw new NotFoundError('Documento no encontrado');
    }

    let readingState = await this.readingStateRepo.findByDocument(
      params.documentId
    );
    ...
    if (readingState.mode !== params.mode) {
      readingState = {
        ...readingState,
        mode: params.mode,
        current_position: 0,
    ...
    if (
      ...
      readingState.current_position = totalUnits > 0 ? 0 : 0;
      await this.readingStateRepo.update(readingState);
    }
